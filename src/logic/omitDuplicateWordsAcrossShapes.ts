...
function getWordsForShape(
  indexListsForShape: number[][],
  letters: LetterQu[],
): Set<string> {
  return new Set(
...
export function omitDuplicateWordsAcrossShapes({
  shapeLookup,
  letters,
}: {
  ...
  const potentialShapeIDs = new Set(allShapeIDs);
  ...
  const deduplicatedShapeLookup: Record<string, number[][]> = {};
